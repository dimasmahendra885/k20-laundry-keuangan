<div>
    ...
    <div class="mb-stack-lg flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <h2 class="text-[40px] font-bold text-primary leading-none">Pengeluaran</h2>
            <p class="text-body-lg text-on-surface-variant mt-2">Catat dan pantau semua biaya operasional laundry.</p>
        </div>
        <div class="flex items-center gap-3">
            <a wire:navigate href="{{ route('pengeluaran.create') }}" class="flex items-center gap-2 px-4 py-2 bg-secondary text-on-secondary rounded-lg font-label-md text-label-md shadow-sm hover:brightness-110 active:scale-95 transition-all">
    ...
    <div class="bg-surface-container-lowest border border-outline-variant rounded-2xl shadow-[0_4px_6px_rgba(0,0,0,0.05)] overflow-hidden">
        ...
        <div class="p-6 border-b border-outline-variant/30 bg-surface-container-low/30">
            <div class="relative max-w-md">
                ...
                <input wire:model.live="search" type="text" placeholder="Cari kategori atau keterangan..." 
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-surface-container-low">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-label-md font-label-md text-on-surface-variant uppercase tracking-wider">Nominal</th>
                ...
                <tbody class="divide-y divide-outline-variant/20">
                    ...
                    <tr class="hover:bg-surface-container-low/30 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-[11px] font-bold bg-surface-variant text-on-surface-variant">
                                {{ $p->kategori }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-body-md font-bold text-error">Rp {{ number_format($p->nominal, 0, ',', '.') }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-body-md text-on-surface-variant line-clamp-1">{{ $p->keterangan }}</p>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <a wire:navigate href="{{ route('pengeluaran.edit', $p->id) }}" class="p-2 text-secondary hover:bg-secondary/10 rounded-lg transition-colors">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-on-surface-variant italic">
